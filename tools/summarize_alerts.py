...
def summarize(csv_path, out_json, gap_s=1.0):
    rows=[]
    ...
    if not rows:
        ...
        return 0
    rows.sort(key=lambda x: (x["src"], x["tid"], x["ts"]))
    events=[]
    t0 = rows[0]["ts"]
    cur=None
    for r in rows:
        if r["fired"]==1:
            if (not cur) or (cur["tid"]!=r["tid"]) or (cur["src"]!=r["src"]) or ((r["ts"]-cur["end_ts"])>gap_s):
                cur={"src":r["src"], "tid":r["tid"], "start_ts":r["ts"], "end_ts":r["ts"], "max_p":r["p"]}
                events.append(cur)
            else:
                cur["end_ts"]=r["ts"]
